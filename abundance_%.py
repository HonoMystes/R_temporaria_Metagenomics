...
phylum_regex = r'o__([A-Za-z0-9_\-\[\]\(\)]+)' # match after p__
phylum_cols = []
phylum_names = []
for col in df.columns:
    m = re.search(phylum_regex, col)
    if m and m.group(1) not in unwanted:
        phylum_cols.append(col)
        phylum_names.append(m.group(1))

# Calculate percentages per sample (row)
phylum_sums = df[phylum_cols].sum(axis=1)
df_percent = df.copy()
for col in phylum_cols:
    df_percent[col] = df[col] / phylum_sums * 100

# Build melted table for calculation
melted = df_percent.melt(
    id_vars=['temp','diet','P'],
    value_vars=phylum_cols,
    var_name='taxonomy',
    value_name='abundance_%'
)

# Add order column
melted[f'{rank}'] = melted['taxonomy'].apply(lambda x: re.search(phylum_regex, x).group(1) if re.search(phylum_regex, x) else 'NA')
...
plt.legend(title='Bacterial order', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
